...
func (*Url) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case url.ForeignKeys[0]: // user_user_id
	...
	for i := range columns {
		...
		case url.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_user_id", values[i])
			} else if value.Valid {
				u.user_user_id = new(uuid.UUID)
				*u.user_user_id = *value.S.(*uuid.UUID)
			}
		}
	}
	return nil
